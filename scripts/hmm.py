...
def getNetworkActivity(data, parcels):
    parcelstuff = pd.read_table('../data/parcel_data.txt')
    parcellation = parcelstuff[parcels]
    roughnetworks = pd.unique(parcellation)
    activities = []
    networks = []
    for network in roughnetworks:
        if parcels == "7networks" and network[:1]=="7":
            netactivity = np.average(data[:,(parcellation==network)],axis=1).reshape((-1,1))
            activities.append(netactivity)
            networks.append(network)
        if parcels == "17networks" and network[:2]=="17":
            netactivity = np.average(data[:,(parcellation==network)],axis=1).reshape((-1,1))
            activities.append(netactivity)
            networks.append(network)
    return networks, np.concatenate(activities,axis=1)
...
def findhiddenstates(data,obsdim,maxstates,cv):
    ...
    while i < cv:
        random.shuffle(data)
        if cv == 1:
            train = data[:math.floor(len(data) * .75)]
            test = data[math.floor(len(data) * .75):]
        else:
...
def loocv(data1, data2,latdim1, latdim2, obsdim):
    ...
    for test in data1:
        ...
        temp = data1.copy()
        del temp[loc]
        model1.fit(temp, method="em", num_em_iters=100)
        if model1.log_likelihood(test) > model2.log_likelihood(test):
            counter += 1
        loc += 1
    accs.append(counter / length)
    loc = 0
    counter = 0
    model1 = ssm.HMM(latdim1, obsdim, observations="diagonal_gaussian")
    model1.fit(data1[:length-1], method="em", num_em_iters=100)
    for test in thursdata:
        model2 = ssm.HMM(latdim2, obsdim, observations="diagonal_gaussian")
        temp = thursdata.copy()
        del temp[loc]
        model2.fit(temp, method="em", num_em_iters=100)
        if model1.log_likelihood(test) < model2.log_likelihood(test):
            counter += 1
        loc += 1
    accs.append(counter / length)
...
#takes a model and a dataset, returns avg occupancy time in each state across run, avg consecutive dwell time in each state
#and avg # of transitions in a run
...
file = open('observations' + str(6), 'rb')
observations = pickle.load(file)
file.close()
mu = observations.mus
#state plots
num_states = 6
half = int(num_states/2)
fig,axs = plt.subplots(2,half,subplot_kw=dict(projection='polar'))
...
while i < half:
    ...
    for t,r in zip(theta,mu[i,:]):
        axs[0,i].annotate(str(round(t*17/(2*math.pi)+1)), xy = [t,r])
    i += 1
while i < num_states:
    axs[1,i-half].set_ylim([-1.5,1.5])
    axs[1,i-half].plot(theta,mu[i,:])
    axs[1,i-half].set_xticklabels([])
    axs[1,i-half].set_yticklabels([])
    for t,r in zip(theta,mu[i-half,:]):
        axs[1,i-half].annotate(str(round(t*17/(2*math.pi)+1)), xy = [t,r])
    i += 1

networkstring = '1 - VisCent\n ' \
                '2 - VisPeri\n' \
                '3 - SomMotA\n' \
                '4 - SomMotB\n' \
                '5 - DorsAttnA\n' \
...
#thursdata = thursdata[math.floor(len(thursdata) / 2):]
    ...
    #get observations and initial states
        if not (os.path.exists('observations' + str(state))):
            hmm = ssm.HMM(state,17,observations="diagonal_gaussian")
            hmm.fit(fulldata,method="em",num_em_iters=500,transonly=False,initialize=True)
            file=open('observations' + str(state),'wb')
            pickle.dump(hmm.observations, file)
            file.close()
            file=open('initialstate' + str(state),'wb')
        ...
        file = open('observations' + str(state),'rb')
        observations = pickle.load(file)
        file.close()
        file = open('initialstate' + str(state),'rb')
        ...
        for data in tuesdata:
            ...
            hmm.fit(data, method="em", num_em_iters=100, transonly=True, initialize=False)
            tuestrans.append(hmm.transitions.transition_matrix)#[np.triu_indices(state)])#[utnodiag(state)])
        ...
        for data in thursdata:
            ...
            hmm.init_state_distn = initialstate
            hmm.fit(data, method="em", num_em_iters=100, transonly=True, initialize=False)
            thurstrans.append(hmm.transitions.transition_matrix)#[np.triu_indices(state)])#[utnodiag(state)])
            occs, dwells, changes = getstats(hmm, [data], state)
...
plt.plot(states,np.average(np.array(allacc),axis=0),label='occupancy time classification')
plt.plot(states,np.ones(np.shape(states))*fcacc,label='functional connectivity classification')
plt.legend()
plt.xlabel('number of hidden states')
plt.ylabel('classification accuracy')
...
for i in range(max_states):
    ...
    accs.append(np.dot(acc,[len(tuesdata),len(thursdata)])/(len(tuesdata)+len(thursdata)))
...
for i in range(max_states):
    for j in range(max_states):
        ...
        accuracies[i,j] = np.dot(accs,[len(tuesdata),len(thursdata)])/(len(tuesdata)+len(thursdata))
        tuesaccs[i,j] = accs[0]
...
for i in range(num_states):
    plt.bar(range(num_bins),height=tuesbinneddata[:,i],bottom=np.sum(tuesbinneddata[:,:i],axis=1))
plt.subplot(122)
for i in range(num_states):
    plt.bar(range(num_bins),height=thursbinneddata[:,i],bottom=np.sum(thursbinneddata[:,:i],axis=1))
plt.show()
